...
import { Box, TextField, IconButton, Popover, InputAdornment } from "@mui/material";
...
const TimeSpinner: React.FC<{
  ...
  const setTime = (h: number, m: number, ap: AmPm) => setInternal({ hour: h, minute: m, ampm: ap });
  ...
  return (
    <Box sx={{
      ...
      {/* Hour */}
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mx: 0 }}>
        <IconButton size="small" sx={{ p: "1px", m: 0, height: 13 }} onClick={() => setTime(hour === 12 ? 1 : hour + 1, minute, ampm)}>
        ...
        />
        <IconButton size="small" sx={{ p: "1px", m: 0, height: 13 }} onClick={() => setTime(hour === 1 ? 12 : hour - 1, minute, ampm)}>
      ...
      {/* Minute */}
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", mx: 0 }}>
        <IconButton size="small" sx={{ p: "1px", m: 0, height: 13 }} onClick={() => setTime(hour, minute === 59 ? 0 : minute + 1, ampm)}>
          <ArrowDropUpIcon fontSize="small" />
        </IconButton>
        <TextField
          value={pad(minute)}
        ...
        />
        <IconButton size="small" sx={{ p: "1px", m: 0, height: 13 }} onClick={() => setTime(hour, minute === 0 ? 59 : minute - 1, ampm)}>
          <ArrowDropDownIcon fontSize="small" />
        </IconButton>
      </Box>

      {/* AM/PM */}
      <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center", ml: 0.2 }}>
        <IconButton size="small" sx={{ p: "1px", m: 0, height: 13 }} onClick={() => setTime(hour, minute, ampm === "AM" ? "PM" : "AM")}>
          <ArrowDropUpIcon fontSize="small" />
        </IconButton>
        <TextField
          value={ampm}
...
const ClockPopover: React.FC<ClockPopoverProps> = ({ label, value, onChange }) => {
        ...
        InputProps={{
          ...
          endAdornment: (
            <InputAdornment position="end">
              <IconButton onClick={handleOpen} size="small" sx={{ color: "#355ad5" }}>
