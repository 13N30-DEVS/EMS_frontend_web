  ...
  const validateShift = useCallback(
    (shift: Shift) => {
      ...
      if (!shift.inTime || !shift.outTime)
        return 'Please select both In-Time and Out-Time.';
      if (
        added.find(
    ...
    >
      ...
      >
        ...
        >
          ...
          <Box sx={{ px: 3, pt: 3, pb: 2 }}>
            ...
            <Box mb={2}>
              <TextField
                ...
                value={newShift.name}
                onChange={e =>
                  setNewShift(p => ({ ...p, name: e.target.value }))
                }
                size='small'
                fullWidth
              />
            ...
            >
              <ClockPopover
                ...
                onChange={val => setNewShift(p => ({ ...p, inTime: val }))}
              />
              <ClockPopover
                label='Out-Time'
                value={newShift.outTime}
                onChange={val => setNewShift(p => ({ ...p, outTime: val }))}
              />
            </Stack>
            ...
            <Box mb={2}>
              <TextField
                ...
                value={newShift.desc}
                onChange={e =>
                  setNewShift(p => ({ ...p, desc: e.target.value }))
                }
                size='small'
                multiline
                minRows={2}
            ...
            <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
              <Button
                ...
                variant='contained'
                sx={{
                  bgcolor: ACTION_GREEN,
                  fontWeight: 600,
                  px: 4,
                  borderRadius: 1,
                  '&:hover': { bgcolor: '#0c8f4f' },
                }}
              >
                Add
              </Button>
          ...
          {error && (
            <Alert severity='error' sx={{ mx: 3, my: 1.5 }}>
              {error}
            </Alert>
          )}
